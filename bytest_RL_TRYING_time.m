function bytest_RL_TRYING_time
clear all
clear workspace
day=100;
n=24;
...
nstrategy=20;
batterysnumber=50;
Qvalue=ones(24,batterysnumber,nstrategy);
...
%%%Reinforcement learning trying%%%
    ...
    for inner=1:nstrategy
        if mod(t0,24)~=0
        sumQvalue(inner)=(Qvalue(t,state,inner));
...
candidate=zeros(1,nstrategy);
    ...
    for inner=1:nstrategy        
        ...
        candidate(inner)=Qvalue(t+1,newstate,inner);
        end
    end

Qvalue(t,state,i)=Qvalue(t,state,i)+aupdate/t*(r+gamma*max(candidate)-Qvalue(t,state,i));
